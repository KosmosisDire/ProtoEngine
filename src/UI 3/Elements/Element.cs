...
public class Element : Drawable
{
    ...
    public Px Width {get; private set;}
    public Px Height {get; private set;}
    public Px InnerWidth {get; private set;}
    public Px InnerHeight {get; private set;}
    public Px OuterWidth {get; private set;}
    public Px OuterHeight {get; private set;}
    public Px OuterLeft {get; private set;}
    public Px OuterRight {get; private set;}
    public Px OuterTop {get; private set;}
    public Px OuterBottom {get; private set;}
    public Px Left {get; private set;}
    public Px Right {get; private set;}
    public Px Top {get; private set;}
    public Px Bottom {get; private set;}
    public Px InnerLeft {get; private set;}
    public Px InnerRight {get; private set;}
    public Px InnerTop {get; private set;}
    public Px InnerBottom {get; private set;}
    public Px CenterX {get; private set;}
    public Px CenterY {get; private set;}
    public Px CalculatedHeight {get; private set;}
    public Px CalculatedWidth {get; private set;}
    ...
    public virtual void BuildBox()
    {
        ...
        var width = CalculatedWidth.pixels;
        var height = CalculatedHeight.pixels;
        ...
        {
            Alignment.Start => (flowDir == Direction.Vertical ? previousSibling?.OuterBottom.pixels + Parent?.ComputedStyle.gap.pixels : Parent?.InnerTop ?? 0) ?? Parent?.InnerTop ?? 0,
            Alignment.Center => Parent?.CenterY.pixels - Height.pixels / 2 ?? 0,
            Alignment.End => Parent?.InnerBottom.pixels - Height.pixels ?? 0,
            _ => Parent?.InnerTop ?? 0
        };

        float left = ComputedStyle.alignSelfX.Value switch
        {
            Alignment.Start => (flowDir == Direction.Horizontal ? previousSibling?.OuterRight.pixels + Parent?.ComputedStyle.gap.pixels : Parent?.InnerLeft ?? 0) ?? Parent?.InnerLeft ?? 0,
            Alignment.Center => Parent?.CenterX.pixels - Width.pixels / 2 ?? 0,
            Alignment.End => Parent?.InnerRight.pixels - Width.pixels ?? 0,
        ...
        Bounds = MarginBounds.Expand(-new Vector2(ComputedStyle.marginLeft.pixels + ComputedStyle.marginRight.pixels, ComputedStyle.marginTop.pixels + ComputedStyle.marginBottom.pixels));
        Bounds += new Vector2(ComputedStyle.marginLeft.pixels, ComputedStyle.marginTop.pixels);

        PaddedBounds = Bounds.Expand(-new Vector2(ComputedStyle.paddingX.pixels, ComputedStyle.paddingY.pixels) * 2)
                            + new Vector2(ComputedStyle.paddingX.pixels, ComputedStyle.paddingY.pixels);
    ...
    {
        var width = ComputedStyle.width.pixels;
        if (ComputedStyle.width.IsUnset)
        {
            width = (Parent?.CalculatedWidth ?? 0) - Parent?.ComputedStyle.paddingX.pixels * 2 ?? 0;
            ...
            if ((Parent?.ComputedStyle.flowDirection ?? Direction.Vertical) == Direction.Horizontal)
            {
                ...
                for (var i = 0; i < siblings.Count; i++)
                {
                    ...
                    if (sibling.ComputedStyle.positionStyle != PositionStyle.Absolute)
                    {
                        if (sibling != this)
                        {
                            width -= Parent?.ComputedStyle.gap.pixels ?? 0;
                            if(!sibling.ComputedStyle.width.IsUnset)
                            {
                                width -= sibling.CalculatedWidth.pixels;
                            }
                        }
            ...
            {
                width -= ComputedStyle.left.pixels + ComputedStyle.right.pixels;
            }
        }

        if (ComputedStyle.marginFit == Fit.Ignore || ComputedStyle.marginFit == Fit.Maximum)
        {
            width += ComputedStyle.marginLeft.pixels + ComputedStyle.marginRight.pixels;
        }
        ...
        if (ComputedStyle.flowDirection == Direction.Horizontal)
        {
            for (var i = 0; i < children.Count; i++)
            {
                ...
                fitWidth += child.CalculatedWidth.pixels;
                if (i != 0) fitWidth += ComputedStyle.gap.pixels;
            }
        }
        else
        {
            ...
            {
                var child = children[i];
                fitWidth = Math.Max(fitWidth, child.CalculatedWidth.pixels);
            }
        }

        width = ProtoMath.Clamp(width, ComputedStyle.minWidth.pixels, ComputedStyle.maxWidth.pixels);
        ...
        width = fit switch
        {
            ...
            Fit.Fit => fitWidth + ComputedStyle.paddingX.pixels * 2,
            Fit.Ignore => width,
            _ => throw new NotImplementedException()
        };

        width = MathF.Max(width, ComputedStyle.minWidth.pixels);
    ...
    {
        var height = ComputedStyle.height.pixels;
        if (ComputedStyle.height.IsUnset)
        {
            height = (Parent?.CalculatedHeight ?? 0) - Parent?.ComputedStyle.paddingY.pixels * 2 ?? 0;
            ...
            if ((Parent?.ComputedStyle.flowDirection ?? Direction.Vertical) == Direction.Vertical)
            {
                ...
                for (var i = 0; i < siblings.Count; i++)
                {
                    ...
                    if (sibling.ComputedStyle.positionStyle != PositionStyle.Absolute)
                    {
                        if (sibling != this)
                        {
                            height -= Parent?.ComputedStyle.gap.pixels ?? 0;
                            if(!sibling.ComputedStyle.height.IsUnset)
                            {
                                height -= sibling.CalculatedHeight.pixels;
                            }
                        }
            ...
            {
                height -= ComputedStyle.top.pixels + ComputedStyle.bottom.pixels;
            }
        }

        if (ComputedStyle.marginFit == Fit.Ignore || ComputedStyle.marginFit == Fit.Maximum)
        {
            height += ComputedStyle.marginTop.pixels + ComputedStyle.marginBottom.pixels;
        }
        ...
        if (ComputedStyle.flowDirection == Direction.Vertical)
        {
            ...
            {
                var child = children[i];
                fitHeight += child.CalculatedHeight.pixels;
                if (i != 0) fitHeight += ComputedStyle.gap.pixels;
            }
        }
        else
        {
            ...
            {
                var child = children[i];
                fitHeight = Math.Max(fitHeight, child.CalculatedHeight.pixels);
            }
        }

        height = ProtoMath.Clamp(height, ComputedStyle.minHeight.pixels, ComputedStyle.maxHeight.pixels);

        var fit = ComputedStyle.contentFitY.Value;
        height = fit switch
        {
            Fit.Minimum => MathF.Max(height, fitHeight) + ComputedStyle.paddingY.pixels * 2,
            Fit.Maximum => MathF.Min(height, fitHeight) + ComputedStyle.paddingY.pixels * 2,
            Fit.Fit => fitHeight + ComputedStyle.paddingY.pixels * 2,
            Fit.Ignore => height,
            _ => throw new NotImplementedException()
        };

        height = MathF.Max(height, ComputedStyle.minHeight.pixels);
