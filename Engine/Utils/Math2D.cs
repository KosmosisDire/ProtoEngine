...
public class Math2D
{
    public static bool LineSegmentsIntersect(Vector2 start, Vector2 end, Vector2 start2, Vector2 end2, out Vector2 intersection, bool considerCollinearOverlapAsIntersect = false)
    {
        ...
        if (!line1Crossline2.IsZero() && (0 <= time1 && time1 <= 1) && (0 <= time2 && time2 <= 1))
        {
            ...
            intersection = start + line1 * time1;
    ...
    public static bool LineSegmentIntersectsCircle(Vector2 start, Vector2 end, Vector2 circleCenter, float circleRadius)
    {
    ...
    public static bool LineSegmentIntersectsBox(Vector2 start, Vector2 end, Vector2 boxCenter, float boxExtent)
    {
        var boxMin = boxCenter - new Vector2(boxExtent, boxExtent);
        var boxMax = boxCenter + new Vector2(boxExtent, boxExtent);
